...
class VerlToolRLHFDataset(RLHFDataset):
    ...
    def _build_messages(self, example: dict):
        ...
        注意：这里 **不** 替换 <image>，让 processor 自动处理。
        替换只在 _build_rollout_messages() 中进行（给 vLLM 用）。
        """
        
        if self._is_internvl3():
            # InternVL3: 保持 <image> 不变，让 processor 自动处理
            messages = deepcopy(example[self.prompt_key])
            
            # 只打印一次调试信息
            if not hasattr(self, '_build_messages_logged'):
                self._build_messages_logged = True
                print(f"[_build_messages] InternVL3: keeping <image> for processor to handle")
