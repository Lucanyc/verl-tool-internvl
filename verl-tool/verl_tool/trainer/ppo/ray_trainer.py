...
import torch
import numpy as np
import random  # 添加import
import json    # 添加import
import os      # 添加import
...
class AgentRayPPOTrainer(RayPPOTrainer):
    def __init__(
        ...
        # 初始化采样相关配置
        self.sample_rate = config.trainer.get("sample_rate", 0.1)  # 默认10%采样率
        #self.sample_save_dir = "/host_home/H200/Online-GRPO-internvl/Online-GRPO-internvl/verl-tool/examples/train/rollout_sample"
        self.sample_save_dir = config.trainer.get("rollout_sample_dir", "./rollout_samples")
       
        # 创建采样目录
        if self.sample_save_dir:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # 关键修改：先处理 reward_extra_info，避免重复添加
            if "reward_extra_info" in result:
                for key, lst in result["reward_extra_info"].items():
                    reward_extra_infos_dict[key].extend(lst)
                    print(f"len reward_extra_infos_dict['{key}']: {len(reward_extra_infos_dict[key])}")
            
            # 只有当 reward_extra_info 中没有 "reward" 键时，才从 scores 添加
            if "reward" not in reward_extra_infos_dict or len(reward_extra_infos_dict["reward"]) == 0:
                reward_extra_infos_dict["reward"].extend(scores)
                print(f"len reward_extra_infos_dict['reward'] (from scores): {len(reward_extra_infos_dict['reward'])}")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # ==================== 添加10%随机采样功能 ====================
                    if self.sample_save_dir:
                        with marked_timer("sample_data", timing_raw, color="blue"):
                            # 解码输入输出
                            inputs = self.tokenizer.batch_decode(batch.batch["prompts"], skip_special_tokens=True)
                            outputs = self.tokenizer.batch_decode(batch.batch["responses"], skip_special_tokens=True)
                            scores = batch.batch["token_level_scores"].sum(-1).cpu().tolist()
                            
                            # 计算要采样的数量：总数据的10%，但至少10条
                            total_samples = len(inputs)
                            num_samples = max(10, int(total_samples * self.sample_rate))
                            # 确保不超过总数据量
                            ...
                            for idx in sample_indices:
                                sample_item = {
                                    ...
                                    "total_samples_in_batch": total_samples,
                                    "sample_index": idx,
                                    "prompt": inputs[idx],
                                ...
                                if reward_extra_infos_dict:
                                    for key, values in reward_extra_infos_dict.items():
                                        if idx < len(values):
                                            sample_item[f"reward_info_{key}"] = values[idx]
                                
                                # 如果有工具调用信息，也添加进去
                                if "__num_turns__" in batch.non_tensor_batch and idx < len(batch.non_tensor_batch["__num_turns__"]):
                                    sample_item["num_turns"] = int(batch.non_tensor_batch["__num_turns__"][idx])
                                
                                sample_data.append(sample_item)
                            
                            # 保存到JSON文件
                            json_filename = f"step_{self.global_steps}_10pct_sampling.json"
                            json_path = os.path.join(self.sample_save_dir, json_filename)
                            ...
                            # 打印采样信息
                            print(f"\n{'='*80}")
                            print(f"[Step {self.global_steps}] Sampled {num_samples}/{total_samples} samples -> {json_filename}")
                            if len(sample_data) > 0:
                                print(f"First sample preview:")
                                print(f"  Prompt: {sample_data[0]['prompt'][:100]}...")
                                print(f"  Response: {sample_data[0]['response'][:100]}...")
                                print(f"  Score: {sample_data[0]['score']:.4f}")
                            print(f"{'='*80}\n")
                    # ==================== 采样功能结束 ====================
