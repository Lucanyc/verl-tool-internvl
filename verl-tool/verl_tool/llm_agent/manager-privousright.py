...
class AgentActorManager:
    ...
    async def _postprocess_responses(self, responses: Union[torch.Tensor, List[str]], action_step: int, rollout_messages: list) -> torch.Tensor:
        ...
        async with self.tokenizer_lock:
            ...
            logger.info("="*50)
            logger.info("Model outputs at step {}:".format(action_step))
            for i, resp in enumerate(responses_str[:2]):  # 只打印前2个
                logger.info(f"Response {i}: {resp[:400]}...")  # 只打印前200字符
            logger.info("="*50)
        ...
        async with self.tokenizer_lock:
            ...
            if not has_multi_modal_data:
                ...
                if self.config.enable_mtrl:
                    ...
                    for i in range(len(next_obs)):
                        if finishs[i] or dones[i]:
                            ...
                            # 添加推理prompt到工具返回内容
                            if self.config.add_reasoning_prompt and next_obs[i]:
                                # 使用配置的模板格式化observation
                                obs_with_prompt = self.config.reasoning_prompt_template.format(obs=next_obs[i])
                                formatted_obs = mtrl_sep.format(obs=obs_with_prompt)
                                
                                # 调试日志
                                if i < 2:  # 只打印前2个样本
                                    logger.info("="*60)
                                    logger.info(f"📝 ADDING REASONING PROMPT TO OBSERVATION {i}:")
                                    logger.info(f"  Original obs: {next_obs[i][:200]}..." if len(next_obs[i]) > 200 else f"  Original obs: {next_obs[i]}")
                                    logger.info(f"  With reasoning prompt: {obs_with_prompt[:400]}..." if len(obs_with_prompt) > 400 else f"  With reasoning prompt: {obs_with_prompt}")
                                    logger.info(f"  Using role: {self.config.mtrl_role}")
                                    logger.info("="*60)
                            else:
                                formatted_obs = mtrl_sep.format(obs=next_obs[i])
                            processed_next_obs.append(formatted_obs)
                        else:
                            # 处理无效action的情况
                            invalid_prompt = "Your action is not valid, please check the format and try again."
                            if self.config.add_reasoning_prompt:
                                # 即使是无效action，也添加推理prompt
                                combined_obs = invalid_prompt + next_obs[i]
                                obs_with_prompt = self.config.reasoning_prompt_template.format(obs=combined_obs)
                                processed_next_obs.append(mtrl_sep.format(obs=obs_with_prompt))
                            else:
                                processed_next_obs.append(mtrl_sep.format(obs=invalid_prompt + next_obs[i]))
                ...
                for k, tool_interact_info_k in enumerate(tool_interact_info):
                    ...
                    # 对多模态情况也添加推理prompt
                    if self.config.add_reasoning_prompt and next_obs_k and valid_action[k] and not (dones[k] or finishs[k]):
                        next_obs_k = self.config.reasoning_prompt_template.format(obs=next_obs_k)
                    
                    if not valid_action[k] and not (dones[k] or finishs[k]):
                        invalid_prompt = "Your action is not valid, please check the format and try again."
                        if self.config.add_reasoning_prompt:
                            next_obs_k = self.config.reasoning_prompt_template.format(obs=invalid_prompt + next_obs_k)
                        else:
                            next_obs_k = invalid_prompt + next_obs_k
    ...
    async def run_llm_loop_async(self, gen_batch: DataProto, **sampling_params: Dict[str, Any]) -> Tuple[Dict, Dict]:
        ...
        # it seems somehow and sometime the non_tensor_batch will be changed by the generate_sequences. so we save a copy and reassign it late
        ...
        for step in range(self.config.max_turns+1):
            ...
            logger.info("="*60)
            logger.info(f"📍 STEP {step}/{self.config.max_turns} STARTING")
            logger.info(f"Active trajectories: {active_mask.sum().item()}/{len(active_mask)}")
            logger.info("="*60)
            ...
            # ===== 添加打印2：模型生成内容 =====
            logger.info(f"🤖 MODEL GENERATED at Step {step}:")
            for i, resp_str in enumerate(responses_str[:3]):  # 打印前3个
                logger.info(f"\n  ========== [Trajectory {i}] ==========")
                
                # 打印简短预览
                #logger.info(f"  Preview: {resp_str[:200]}{'...' if len(resp_str) > 200 else ''}")
                
                # 打印完整响应（前2000字符）
                logger.info(f"  Full Response (first 2000 chars):")
                logger.info(f"  {resp_str[:2000]}")
                
                # 检查关键内容
                has_tool_call = '</tool_call>' in resp_str
                has_think = '<think>' in resp_str
                has_answer = '<answer>' in resp_str
                
                # 如果有answer标签，提取并显示
                if has_answer:
                    import re
                    answer_match = re.search(r'<answer>(.*?)</answer>', resp_str, re.DOTALL)
                    if answer_match:
                        logger.info(f"  📌 Extracted Answer: {answer_match.group(1).strip()}")
                
                # 如果有think标签，显示推理部分的前300字符
                if has_think:
                    import re
                    think_match = re.search(r'<think>(.*?)</think>', resp_str, re.DOTALL)
                    if think_match:
                        think_content = think_match.group(1).strip()
                        logger.info(f"  💭 Reasoning (first 300 chars): {think_content[:300]}...")
                
                # 统计信息
                logger.info(f"  📊 Response Stats:")
                logger.info(f"    - Total length: {len(resp_str)} chars")
                logger.info(f"    - Has </tool_call>: {has_tool_call}")
                logger.info(f"    - Has <think>: {has_think}")
                logger.info(f"    - Has <answer>: {has_answer}")
                logger.info(f"    - Do action: {do_actions[i] if i < len(do_actions) else 'N/A'}")
                
                # 如果响应看起来被截断（在tool_call后很短）
                if has_tool_call and len(resp_str) < 500:
                    tool_call_idx = resp_str.rfind('</tool_call>')
                    after_tool_call = resp_str[tool_call_idx+12:] if tool_call_idx != -1 else ""
                    logger.info(f"  ⚠️ WARNING: Response might be truncated!")
                    logger.info(f"    - Content after </tool_call>: '{after_tool_call}'")
    ...
    async def interact_with_tool_server(
            ...
            # ===== 打印发送给工具服务器的内容 =====
            logger.info("="*60)
            logger.info("📤 SENDING TO TOOL SERVER:")
            logger.info(f"  Active UIDs: {active_uids[:3]}..." if len(active_uids) > 3 else f"  Active UIDs: {active_uids}")
            logger.info(f"  Number of actions: {len(responses)}")
            for i, (uid, action, finish) in enumerate(list(zip(active_uids, responses, finishs))[:3]):
                logger.info(f"  [{i}] UID: {uid}")
                logger.info(f"      Action: {action[:200]}..." if len(action) > 200 else f"      Action: {action}")
                logger.info(f"      Finish: {finish}, Do_action: {not finish}")
            logger.info(f"  Is last step: {is_last_step}")
            logger.info("="*60)
            ...
            # ===== 打印原始响应 =====
            logger.info("="*60)
            logger.info("📥 RAW TOOL SERVER RESPONSE:")
            logger.info(f"  Response keys: {list(response.keys())}")
            logger.info(f"  Number of observations: {len(response.get('observations', []))}")
            logger.info("="*60)
            ...
            # ===== 详细打印工具响应 =====
            logger.info("="*60)
            logger.info(f"🔧 TOOL RESPONSE DETAILS:")
            for i, obs in enumerate(active_observations[:3]):  # 打印前3个
                logger.info(f"\n  [Observation {i}]:")
                if obs:  # 只打印非空观察
                    if isinstance(obs, str):
                        logger.info(f"    Type: string")
                        logger.info(f"    Length: {len(obs)} chars")
                        logger.info(f"    Content: {obs[:500]}..." if len(obs) > 500 else f"    Content: {obs}")
                    elif isinstance(obs, dict):
                        logger.info(f"    Type: dict")
                        logger.info(f"    Keys: {list(obs.keys())}")
                        if 'obs' in obs:
                            obs_content = obs['obs']
                            logger.info(f"    'obs' length: {len(obs_content)} chars")
                            logger.info(f"    'obs' content: {obs_content[:500]}..." if len(obs_content) > 500 else f"    'obs' content: {obs_content}")
                        if 'reward' in obs:
                            logger.info(f"    'reward': {obs['reward']}")
                        if 'image' in obs:
                            logger.info(f"    'image': {type(obs['image'])} - {len(obs['image']) if isinstance(obs['image'], list) else 'single'}")
                        if 'video' in obs:
                            logger.info(f"    'video': {type(obs['video'])} - {len(obs['video']) if isinstance(obs['video'], list) else 'single'}")
                        # 打印其他键
                        other_keys = [k for k in obs.keys() if k not in ['obs', 'reward', 'image', 'video']]
                        if other_keys:
                            logger.info(f"    Other keys: {other_keys}")
                else:
                    logger.info(f"    Empty observation")
            logger.info("="*60)
            ...
            logger.info(f"  Total observations to process: {len(next_obs)}")
            
            # postprocess next_obs
            processed_next_obs = []
            rewards = []
            tool_interact_info = []
            active_idx = 0
            ...
            for i, obs in enumerate(next_obs):
                ...
                if i < 3 and obs:
                    logger.info(f"\n  [Processing {i}]:")
                    logger.info(f"    Original type: {type(obs).__name__}")
                ...
                if isinstance(obs, str):
                    ...
                    if i < 3 and obs:
                        logger.info(f"    Processed as: string")
                        logger.info(f"    Reward: None")
                    ...
                    if i < 3:
                        logger.info(f"    Processed as: dict")
                        logger.info(f"    Extracted 'obs': {_obs[:100]}..." if len(_obs) > 100 else f"    Extracted 'obs': {_obs}")
                        logger.info(f"    Extracted 'reward': {_reward}")
                        if len(obs) > 2:  # 如果有其他键
                            logger.info(f"    Other data preserved: {[k for k in obs.keys() if k not in ['obs', 'reward']]}")
            ...
            logger.info(f"  Processed observations: {len(processed_next_obs)}")
            logger.info(f"  With rewards: {len([r for r in rewards if r is not None])}")
            logger.info(f"  Active trajectories: {sum(active_mask)}")
            logger.info(f"  Valid actions: {sum(valid_action)}")
            logger.info(f"  Finished: {sum(_finishs)}")
            logger.info("="*60)
