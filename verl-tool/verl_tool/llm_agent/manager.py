...
class AgentActorManager:
    def __init__(
        ...
        # Fixed: Directly use "function" role for InternVL3
        if self.config.mtrl_sep is None:
            messages = [{"role": "function", "content": "{obs}"}]
            ...
            # ===== 添加打印语句 =====
            logger.info("="*60)
            logger.info("🔍 MTRL_SEP FORMAT CHECK:")
            logger.info(f"Raw mtrl_sep (repr): {repr(self.config.mtrl_sep)}")
            logger.info(f"Raw mtrl_sep (len): {len(self.config.mtrl_sep)} chars")
            logger.info("-"*30)
            logger.info("Formatted mtrl_sep:")
            logger.info(self.config.mtrl_sep)
            logger.info("-"*30)
            # 逐字符打印以检查特殊字符
            logger.info("Character breakdown:")
            for i, char in enumerate(self.config.mtrl_sep[:50]):  # 打印前50个字符
                if char == '\n':
                    logger.info(f"  [{i}]: '\\n' (newline)")
                elif char == ' ':
        ...
        async with self.tokenizer_lock:
            ...
            if not has_multi_modal_data:
                ...
                if self.config.enable_mtrl:
                    ...
                    for i in range(len(next_obs)):
                        if finishs[i] or dones[i]:
                            ...
                            # Fixed: Only include tool output, no reasoning prompt
                ...
                for k, tool_interact_info_k in enumerate(tool_interact_info):
                    ...
                    # Fixed: No reasoning prompt for multimodal case
                    ...
                    if content_list and not dones[k] and not finishs[k]:
                        # Fixed: Directly use function role for multimodal
                        next_obs_message = [{"role": "function", "content": content_list}]
