...
conda run -n easyocr_env --no-banner \
    bash -c "cd ${VERL_TOOL_DIR} && VERL_APPLY_QWEN25VL_PATCH=0 CUDA_VISIBLE_DEVICES=6 \
...
conda run -n gllava --no-banner \
    bash -c "cd ${VERL_TOOL_DIR} && VERL_APPLY_QWEN25VL_PATCH=0 CUDA_VISIBLE_DEVICES=7 \
...
conda run -n chartmoe-env --no-banner \
    bash -c "cd ${VERL_TOOL_DIR} && CUDA_VISIBLE_DEVICES=0 \
